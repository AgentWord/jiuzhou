...
public class ProjectManagerService {
	...
	// -----------------------------项目信息---------------------------
		...
		// 添加项目信息
		@SuppressWarnings("unchecked")
		public Map<String, Object> addProjectInfo(ProjectManagement projectManagement) {
			String userName = request.getUserPrincipal().getName();
			Session session = sessionFactory.getCurrentSession();
			Map<String, Object> roleInfoResults = new HashMap<String, Object>();
			try {
				double remain= projectManagement.getProMoney()-projectManagement.getProIncome();
				System.out.println(remain);
				
				projectManagement.setProRemain(remain);
				projectManagement.setProStatus("已提交");
				projectManagement.setProStartTime(projectManagement.getProStartTime().substring(0,10));
				projectManagement.setProEndTime(projectManagement.getProEndTime().substring(0,10));
