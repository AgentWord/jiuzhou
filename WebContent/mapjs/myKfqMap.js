...
require([
    ...
    ready(function () {//65.3447265625,4.21875, 135.7470703125, 53.61328125
        ...
        //---------Ext JS 事件----------
        //初始化选择图层
        comboBoxLayerSelect = Ext.getCmp('survey_layerUrls_Combo2');
        //图层选择变化时
       /* */
        ...
        function onComboBoxChange(comboBox, newValue, oldValue, eOpts) {
            //加载图层
            var url = newValue;//comboBox.getValue()

            //var url = "http://localhost:6080/arcgis/rest/services/kaifaqu2013/MapServer/8";
            console.log("地图地址："+url);
            if (!url) {
                return;
            }
            if (layer) {
            ...
            if (layer) {
                ...
                function featureLayerClick(event) {
                    ...
                    if (!extInfoWindowGrid) {
                        extInfoWindowGrid = Ext.getCmp("mapInfoWindowGrid1");
                    }
        ...
        grid = Ext.getCmp('survey_LanduseGrid1');
        if (grid) {
        ...
        //地图panel大小变化时
        mapPanel = Ext.getCmp('survey_Landuse_MapPanel1');
        if (mapPanel) {
